...
> - ドングルへの書き込みは nrfutil の `nrf5sdk-tools` コマンドで行う。`pkg generate` でパッケージを作り、続けて `dfu usb-serial` で書き込む。ここでいう nrfutil は、機能を `nrfutil install <名前>` で後から追加していく現行の単一実行ファイル版を指す（Nordic はこれを "unified nrfutil" と呼ぶ）。旧来の Python 版（`pip install nrfutil` で導入）にあった `nrfutil pkg` / `nrfutil dfu` は、現行版には存在しない。
