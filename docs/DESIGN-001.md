...
| `setup` | check-os, install-tools, build-firmware | **ソフトウェア環境構築（実機不要）。** 前提確認→ツール導入→ファームウェアビルドまでをソフト工程のみで一括実行する（明示呼び出し）。実機書き込み＆検証は `deploy` が担う（→ DL-9）。各依存ターゲットが個別に冪等であるため setup の再実行も冪等。 | 依存先がすべて冪等であることに依存する。setup 自体は状態を持たない。 |
| `deploy` | flash-dk, flash-sniffer-dongle, verify | **実機へ書き込み＆検証（要 DK＋ドングル接続）。** DK へのフラッシュ→ドングルへの Sniffer FW 書き込み→読み取り専用検査を一括実行する。非並列 make では prerequisite が左→右順に実行されるため flash-dk→flash-sniffer-dongle→verify の順に走る。verify から flash 依存を外したことで flash-dk の二重実行を防ぐ（→ DL-9）。 | 依存先がすべて（実質）冪等であることに依存する。deploy 自体は状態を持たない。 |
...
| `verify` | — | DK が BLE Peripheral として広告していること、Wireshark に Sniffer インタフェースが出現していることを検査する。フラッシュ済みを前提とする読み取り専用ターゲット（書き込みは `deploy` が行う → DL-9）。 | 読み取り専用の検査のみ。状態を変更しない。 |
...
| DL-9 | `setup` をソフト/ハードに分割する（`setup`=実機不要のソフト工程、`deploy`=実機書き込み＆検証） | **従来 `setup` は末尾に `verify`（→`flash-dk`/`flash-sniffer-dongle`）を従え、物理デバイス（nRF52840 DK / Dongle）と J-Link/nrfjprog に依存していたため、実機なしでは完走できなかった。** 環境構築の完了確認（ツール導入＋ファームウェアビルドが通るか）すら実機が無いと検証できない状態だった。<br>**対策:** `setup` をハードウェア非依存部分のみ（`check-os install-tools build-firmware`）に絞り、実機が無くても完走させる。実機への書き込み＆検証は新ターゲット `deploy`（`flash-dk flash-sniffer-dongle verify`）へ分離した。あわせて `verify` から `flash-dk`/`flash-sniffer-dongle` 依存を**外し**、読み取り専用（フラッシュ済み前提）の独立ターゲットにした。<br>**二重フラッシュ防止:** 全ターゲットは `.PHONY` のため、`flash-dk` を `deploy` と `verify` の両方の prerequisite にすると flash-dk が 2 回実行される。よって `verify` から flash 依存を外すことを必須とした。`deploy` の prerequisite は非並列 make の左→右順により flash-dk→flash-sniffer-dongle→verify の順で 1 回ずつ実行される。<br>**CI 追従:** parse-lint の依存グラフ検証を 2 系統（`make -n setup` はソフト工程 needle のみ・flash/verify が**含まれないこと**も確認、`make -n deploy` は flash/verify needle）に分割した。 | UX / 構成（ユーザー承認済み） |
