...
capture: init ## ② アナライザ: ドングルに Sniffer を焼き Wireshark で DK↔iPhone を観測
	...
	@echo "  5) iPhone の nRF Connect で 'Nordic_UART_Service' に Connect し、各フェーズを読む:"
	@echo "     5-1) 接続が成立すると、一覧の Source/Destination が Central <-> Peripheral 表記に変わる"
	@echo "          （= Advertise→Connect は完了。CONNECT_IND が境目）。"
	@echo "     5-2) 直後の一覧は 'Empty PDU'（接続維持の空パケット）で埋まる。探索は接続直後に"
	@echo "          終わって上へ流れているだけなので慌てない。"
	@echo "     5-3) 表示フィルタ欄に  btatt  と打って Enter。Empty PDU が消え、ATT だけが残る。"
	@echo "     5-4) Info 列を上から読むと、各フェーズが並ぶ:"
	@echo "          - 'Exchange MTU Request/Response'        … MTU 交渉（小さい方の MTU に合意）"
	@echo "          - 'Read By Group Type Request/Response'  … サービス探索（'Nordic UART Service' を発見）"
	@echo "            ※ 末尾の 'Error: Attribute Not Found' は『もう無い』＝探索完了の正常な合図"
	@echo "          - 'Read By Type' / 'Find Information'     … 特性・Descriptor の探索"
	@echo "  これら（接続→MTU 交渉→GATT Discovery）が並べば『観測手段の確立』は完了です。"
	@echo "  6)（任意・裏取り）iPhone で 'Nordic UART Rx' に hello を Write すると、一覧に"
	...
	@echo "     ＝ Phase 1 でシリアルに出た同じバイトを電波でも確認（Swift/シリアルの裏取り）。"
	@echo ""
	@echo "  困ったら:"
